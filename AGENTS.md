...
Image input preference: Support pasting copied images or screenshots directly into the post/article body with Ctrl+V or Cmd+V, alongside local file selection. Share media validation and persistence between both paths, preserve plain-text paste, and use paste-event clipboard data so HTTP IP + port deployments work without clipboard-read permissions.
