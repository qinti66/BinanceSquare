...
启动成功后，终端显示访问地址和首次生成的登录信息。浏览器打开 **http://服务器IP:8081**，在网页登录页输入 `.env` 中的 `ADMIN_USER` 和 `ADMIN_PASSWORD` 即可。网页、API 和媒体使用同一个端口。关闭 SSH 不会停止服务。

同一管理员可在多个浏览器同时登录，各浏览器分别保存登录会话，不会互相挤下线；已同步的账号、草稿和发布历史共享。登录会话有效期为 12 小时，服务器重启后需重新登录。网页登录不依赖浏览器的 HTTP Basic 弹窗，API 客户端仍兼容 HTTP Basic 认证。
